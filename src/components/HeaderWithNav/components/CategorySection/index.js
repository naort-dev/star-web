...
import { fetchCelebrityList } from '../../../../pages/landing/actions/getCelebList';
...
const CategorySection = (props) => {
  ...
  const updateMainCategory = (title, value, subCategories) => () => {
    ...
    props.fetchCelebrityList(0, true, 'stars');
  };
  return (
    <CategoryStyled>
      <CategoryStyled.Item
        ...
        data-value="Featured"
      >
        <Link to='/' className="category-label">Featured</Link>
      </CategoryStyled.Item>
      {
        professionsList.map((profession) => {
          return (
            ...
            >
              <Link to='/' className="category-label">{profession.title}</Link>
            </CategoryStyled.Item>
          );
        })
      }
