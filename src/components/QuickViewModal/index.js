...
const QuickViewModal = props => {
  ...
  const isBookable =
    ...
    props.celebDetails.remaining_limit > 0 &&
    !props.isStar;
  const starData = [
    {
      size: '105px',
  ...
  useEffect(() => {
    ...
    const isPresentCelebDetails =
      !isEmpty(userDetails) && !isEmpty(celebDetails);
    const deferFetchCeleb =
      isPresentCelebDetails &&
      userDetails.user_id === props.quickViewModal.data;
  ...
  const renderDescription = () => {
    if (
      ...
      return (
        <React.Fragment>
          <strong>{getShortName()}</strong> is temporarily unavailable. Come
          back later.
        </React.Fragment>
      );
    ...
    return (
      <React.Fragment>
        ...
        <strong>{getShortName()}</strong> for{' '}
        <strong>
          ${props.celebDetails.rate && parseInt(props.celebDetails.rate, 0)}
        </strong>
      </React.Fragment>
    );
  };
        ...
        <React.Fragment>
          <QuickViewStyled>
            <QuickViewStyled.VideoContainer
              className={!showVideo && 'image-only'}
            >
              {showVideo ? (
                <VideoRender
                  variableWidth
                ...
                <QuickViewStyled.Avatar
                  size={200}
                  imageUrl={
                    props.userDetails.avatar_photo &&
                    props.userDetails.avatar_photo.thumbnail_url
                  }
                />
              )}
            </QuickViewStyled.VideoContainer>
            <QuickViewStyled.Content>
              ...
              {props.celebDetails.average_response_time !== '' && (
                <React.Fragment>
                  ...
                  <QuickViewStyled.SubDescription>
                    {props.celebDetails.average_response_time}
                  </QuickViewStyled.SubDescription>
                </React.Fragment>
              )}
              ...
              <QuickViewStyled.MiniDescription
                onClick={props.toggleQuickView(false)}
                to={`/${props.userDetails.user_id}`}
              >
                Read full profile
              </QuickViewStyled.MiniDescription>
            </QuickViewStyled.Content>
          ...
          >
            ...
            {isBookable && (
              <QuickViewStyled.ActionSection>
                ...
                <PrimaryButton className="action-button">
                  Book Now
                </PrimaryButton>
              </QuickViewStyled.ActionSection>
            )}
...
const mapDispatchToProps = dispatch => ({
  ...
  updateFavouritesQueue: (id, follow) =>
    dispatch(updateFavouritesQueue(id, follow)),
