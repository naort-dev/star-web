...
import { updateProfilePhoto, setProfilePicToState } from '../../../../store/shared/actions/updateProfilePhoto';
...
class NameAndPhoto extends React.Component {
  state = {
    name: this.props.userDetails.settings_userDetails.nick_name? this.props.userDetails.settings_userDetails.nick_name : '',
    currentExif: null,
    verificationDisable: false,
    cropper: false,
    finalImage: this.props.userDetails.settings_userDetails.avatar_photo.thumbnail_url,
    finalFile: null,
    cropImage: null,
    extension: null,
    takePicture: false,
  ...
  uploadImage = () => {
    ...
    const finalUserDetails = {
      ...
      user_details: {
        ...
        show_nick_name: this.state.name && this.state.name.trim('') ? true : false,
      },
    }; 
    ...
    return (
      ...
      { !this.state.takePicture && !this.state.cropper &&
        ...
        <Layout.ButtonWrapper className="align-center">
          <PrimaryButton className='save-button' onClick={this.primaryButtonClick} >
            Save             
          </PrimaryButton>
